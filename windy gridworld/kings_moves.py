...
actions = [up, down, left, right, ur, dr, dl, ul]
...
class Windy:
    def __init__(self):
        ...
        sar_q_value = np.zeros((grid_height, grid_width, 8), dtype=float)
        max_episodes = 500
    ...
    def main_logic(self, sar_q_value, max_episodes):
        ...
        plt.ylabel("Episodes")
        plt.savefig("Plot(kings_moves).png")
        plt.close()
    ...
    def step(self, sar_curr_state, action):
        ...
        if action == up:
            ...
            return [max(i - 1 - wind[j], 0), min(j + 1, grid_width - 1)]
        elif action == dr:
            return [max(min(i + 1 - wind[j], grid_height - 1), 0), min(j + 1, grid_width - 1)]
        elif action == dl:
            return [max(min(i + 1 - wind[j], grid_height - 1), 0), max(j - 1, 0)]
        elif action == ul:
