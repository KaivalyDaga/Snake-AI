...
class Agent:
    def __init__(self):
        ...
        self.qmatrix_old = np.zeros((8, 255, 4), dtype=int)
        self.qmatrix_new = np.zeros((8, 255, 4), dtype=int)
    ...
    def plot_graphs(self, __fruit):
        ...
        plt.savefig("fruits_episodes.png")
        plt.close()
        return
    ...
    def episode(self, __fruit, num):
        ...
        while check:
            ...
            clock.tick(100000)
        if (num > 1 and np.allclose(self.qmatrix_new, self.qmatrix_old, 0, 0.01)) or num > 2500:
            np.save("qmatrix_6", self.qmatrix)
